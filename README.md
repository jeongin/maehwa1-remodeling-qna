...
1. 계정은 조합이 미리 등록합니다. 화면에서 자동 생성되지 않습니다
2. 곧바로 **비밀번호 변경 창이 강제로** 뜹니다. 변경 전에는 닫을 수 없습니다
3. 이후부터는 동·호수와 **변경한 비밀번호**로 입장합니다
4. 헤더의 **🔑 비밀번호 변경** 버튼으로 언제든 다시 바꿀 수 있습니다

강제 변경 여부는 `users/{uid}.pwChanged` 플래그로 판단합니다. 등록되지 않은 동·호수로는 입장할 수 없고, 명부 문서가 없는 계정은
...
관리자는 게시판에서 질문을 직접 등록할 수 있고(답변까지 함께), 기존 질문의
제목·카테고리·내용·작성자·답변을 모두 수정할 수 있습니다. 관리자가 등록한
질문은 작성자가 관리자 계정이므로 해당 조합원 화면에는 보이지 않습니다.
